    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/95 backdrop-blur-md">
      <div className="site-container flex h-20 items-center justify-between gap-6">
        <Link to="/" className="group flex items-center gap-3" aria-label="Afeez Amusa Onireke home">
          <span className="flex size-10 items-center justify-center border border-gold bg-ink font-serif text-sm text-gold transition-colors group-hover:bg-ink-soft">
            AAO
          </span>
          <span className="hidden max-w-36 text-[0.72rem] font-bold uppercase leading-tight tracking-[0.13em] text-foreground sm:block">
            Afeez Amusa Onireke
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-5 xl:flex" aria-label="Primary navigation">
          {navItems.map((item) => (
            <Link
              ...
              className="text-[0.67rem] font-semibold uppercase tracking-[0.12em] text-muted-foreground transition-colors hover:text-foreground"
          ...
          <Sheet>
            ...
            <SheetContent className="border-border bg-background p-0">
              <SheetHeader className="border-b border-border px-6 py-7 text-left">
                <SheetTitle className="font-serif text-2xl">Afeez Amusa Onireke</SheetTitle>
                <SheetDescription>Professional ecosystem</SheetDescription>
              </SheetHeader>
              <nav className="flex flex-col px-6 py-5" aria-label="Mobile navigation">
                {navItems.map((item, index) => (
                  <SheetClose asChild key={item.to}>
                    <Link
                      to={item.to}
                      className="flex items-center justify-between border-b border-border py-3 text-sm font-semibold uppercase tracking-[0.12em]"
                    >
    ...
    <footer className="bg-ink text-paper">
      <div className="site-container py-14 md:py-20">
        <div className="grid gap-12 border-b border-paper/15 pb-12 md:grid-cols-[1.3fr_2fr] md:pb-16">
          <div>
            <div className="mb-6 flex size-12 items-center justify-center border border-gold font-serif text-gold">AAO</div>
            <h2 className="max-w-md font-serif text-3xl leading-tight md:text-4xl">The digital headquarters of a multidisciplinary professional ecosystem.</h2>
          </div>
